...
class MenuBar {
    ...
    func load() -> [MenuItem] {
        ...
        return MenuGetter.loadSync(menuBar: menuBar, options: MenuGetterOptions())
    }
    
    func loadAsync() -> [MenuItem] {
        guard let menuBar = self.menuBar else {
            return []
        }
        return MenuGetter.loadAsync(menuBar: menuBar, options: MenuGetterOptions())
    }
}
